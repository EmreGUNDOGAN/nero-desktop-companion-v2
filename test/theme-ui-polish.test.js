...
test('başlık kontrolleri drag bölgesinden fiziksel olarak ayrıdır ve no-drag kalır', () => {
  ...
  assert.ok(dragAt > 0 && controlsAt > dragAt && headerAt > controlsAt, 'drag-zone ve window-controls header dışında shell sibling olmalı');
  for (const id of ['settings-button', 'pin', 'minimize', 'close']) {
    assert.match(html, new RegExp(`id="${id}"`));
  }
  assert.match(panelCss, /\.top\s*\{[\s\S]*?-webkit-app-region:\s*no-drag;/);
  assert.match(panelCss, /\.panel-drag-zone\s*\{[\s\S]*?-webkit-app-region:\s*drag;[\s\S]*?right:\s*205px;/);
  assert.match(panelCss, /\.window-controls\s*\{[\s\S]*?-webkit-app-region:\s*no-drag;[\s\S]*?z-index:\s*100;/);
