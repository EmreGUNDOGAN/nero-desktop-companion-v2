...
let settingsStore, notesStore, todosStore, moodStore, statsStore, moodLogStore, archiveStore, jarStore;
let themes, dialogue, mood, timer, stats, journal;
let homeCache = null;
let quoteOffset = 0;
let resize = null;
let petTimes = [];
...
// Ana sayfadaki Nero lafı ve günün sözü (panel her açıldığında yenilenir).
function buildHome() {
  const sum = stats.summary();
  const moodLog = journal.last30();
  const archive = journal.thisMonth(8);
  let letter = null;
  const wk = isoWeekKey();
  if (settings().lastLetterWeek !== wk && sum.week.some((d) => d.focus > 0 || d.todos > 0)) {
  ...
  const vars = {
    todos: sum.today.todos, focus: sum.today.focus, streak: sum.streak,
    quit: sum.totals.timersQuit, done: sum.totals.timersDone,
    days: sum.daysTogether, hours: Math.floor(sum.totals.focusMin / 60)
  };
  let category = 'home_general';
  if (sum.today.todos === 0 && sum.today.focus === 0 && hour >= 11) category = 'home_nothing_today';
  else if (sum.today.todos >= 3 || sum.today.focus >= 50) category = 'home_good_day';
  else if (sum.streak >= 3 && chance(0.5)) category = 'home_streak';
  else if (sum.totals.timersQuit >= 3 && sum.totals.timersQuit > sum.totals.timersDone && chance(0.6)) category = 'home_quitter';
  const line = dialogue.pick(category, vars) || dialogue.pick('home_general', vars);
  ...
  homeCache = { jab: line ? line.text : '', quote, moodLog, archive, letter: letter || homeCache?.letter || null };
}
...
function wakeNap(reason) {
  ...
  updateBaseline(true);
  if (reason === 'click' || reason === 'drag' || reason === 'pet') stats.award('uyandirdin');
...
function napTick() {
  ...
  if (mood.state.napping) {
    ...
    if (now >= nextSnoreAt) {
      ...
      if (chance(0.5)) say('snore', {}, { interrupt: false });
    }
    return;
  }
  const night = currentOutfit() === 'pajama';
...
function petNero() {
  if (wakeNap('pet')) { mood.interact('pet'); stats.pet(); broadcastState(); return; }
  const now = Date.now();
...
function registerIpc() {
  ...
  ipcMain.handle('notes:save', (_e, note) => {
    ...
    if (existing) {
      ...
      saved = { id: uid(), body, createdAt: now, updatedAt: now };
      stats.noteCreated();
  ...
  ipcMain.handle('todos:add', (_e, text, time) => {
    ...
    const todo = { id: uid(), text: clean, done: false, createdAt: Date.now(), doneAt: null, remindAt: parseRemindTime(time), reminded: false };
  ...
  ipcMain.handle('todos:setReminder', (_e, id, time) => {
    const todos = todosStore.get();
    const todo = todos.find((t) => t.id === id);
    if (todo) { todo.remindAt = parseRemindTime(time); todo.reminded = false; todosStore.set(todos); }
    broadcastState();
    return todo;
  });
  ...
  ipcMain.handle('todos:clearDone', () => {
    todosStore.set(todosStore.get().filter((t) => !t.done));
    broadcastState();
    return true;
  });
  ...
  ipcMain.handle('quick:addTodo', async (_e, text) => {
    const clean = String(text || '').trim().slice(0, 300);
    if (clean) { todosStore.set([...todosStore.get(), { id: uid(), text: clean, done: false, createdAt: Date.now(), doneAt: null, remindAt: null, reminded: false }]); stats.markActive(); }
    broadcastState();
    return true;
  });
  ipcMain.handle('quick:addNote', async (_e, text) => {
    const body = String(text || '').trim().slice(0, 20000);
    if (body) { notesStore.set([{ id: uid(), body, createdAt: Date.now(), updatedAt: Date.now() }, ...notesStore.get()]); stats.noteCreated(); }
    broadcastState();
    return true;
  });
  ...
  ipcMain.handle('jar:add', (_e, text) => {
    ...
    if (item) reactToInteraction('note_add', () => say('jar_add', {}, { force: false }));
    broadcastState();
    return item;
  });
...
function wireTimer() {
  ...
  timer.on('paused', () => say('timer_pause'));
  timer.on('resumed', () => { if (chance(0.8)) say('timer_resume'); });
  timer.on('done', ({ minutes, label }) => {
    stats.focus(minutes, true);
  ...
  timer.on('cancelled', ({ progress, minutes }) => {
    stats.focus(minutes * progress, false);
...
function startLoops() {
  ...
  setInterval(() => {
    ...
    lastTick = now;
    mood.tick(dt, powerMonitor.getSystemIdleTime(), screenLocked);
    stats.markActive();
  ...
  setInterval(() => {
    ...
    if (mood.stage === 'content' && chance(0.12)) {
      ...
      if (j) say('jar_recall', { text: truncate(j.text, 60) }, { interrupt: false });
    } else {
...
// Yeni rozet: aynı anda birden fazla gelirse arka arkaya söylesin.
let unlockDelay = 0;
...
function notify(title, body) {
  ...
  new Notification({ title, body, icon: iconPath, silent: !settings().sound }).show();
}
...
async function importData() {
  ...
  if (data.stats && typeof data.stats === 'object') { statsStore.set(data.stats); stats = new Stats(statsStore); stats.onUnlock = handleUnlock; }
  broadcastState();
  return { ok: true };
}
...
function installUpdateNow() {
  ...
  isQuitting = true;
  for (const store of [settingsStore, notesStore, todosStore, moodStore, statsStore, moodLogStore, archiveStore, jarStore]) store?.flush();
...
app.on('before-quit', () => {
  ...
  for (const store of [settingsStore, notesStore, todosStore, moodStore, statsStore]) store?.flush();
});
